...
// opening sweep, once the car settles and the cards are first revealed —
// "way faster than now", plain (no motion-blur flourish)
const FAST_SPEED = 0.45;
const FAST_PHASE_DURATION = 2.2; // seconds before easing back to the normal drift
...
export default function SpiralGallery({
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      if (introDoneRef.current) fastPhaseElapsed += dt;
      const baseSpeed =
        fastPhaseElapsed < FAST_PHASE_DURATION ? FAST_SPEED : NORMAL_SPEED;
